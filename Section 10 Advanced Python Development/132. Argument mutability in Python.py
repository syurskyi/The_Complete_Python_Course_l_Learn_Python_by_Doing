...
"""
As you can see if you run that code, `friends` has the value of `friends_last_seen` when we run it—it’s 
...
"""
Thus, it’s all up to whether `__add__` and `__iadd__` create new objects or modify the existing objects. 
It’s easy to check their implementations by doing this:
...
Just remember, when you pass something to a function you can mutate that thing—then the value outside 
the function will have changed too.
